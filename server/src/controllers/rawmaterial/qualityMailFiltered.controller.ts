...
export class RMQualityMailFilteredController {
    ...
    static async mailFilteredQualityReports(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (fromDate || toDate) {
                where.dateOfReport = {};
                if (fromDate) where.dateOfReport.gte = new Date(fromDate);
                if (toDate) where.dateOfReport.lte = new Date(toDate);
            }
            ...
            if (reports.length === 0) {
                res.status(404).json({
                    success: false,
                    error: 'No quality reports found to mail for the given filters',
                });
                return;
            }
            ...
            const parameterHeaders = parametersList.map(param =>
                `<th style="padding: 10px; border: 1px solid #ddd; background-color: #2C3E50; color: white;">${param} - Std</th>
                <th style="padding: 10px; border: 1px solid #ddd; background-color: #2C3E50; color: white;">${param} - Res</th>`
            ).join('');
            ...
            const htmlTable = `
                <div style="font-family: Arial, sans-serif; padding: 20px;">
                    <h2 style="color: #2C3E50;">Filtered RM Quality Reports Export</h2>
                    <p>The filtered RM Quality Reports have been exported successfully.</p>
            ...
            await sendTransactionalEmail({
                ...
                subject: 'Filtered RM Quality Reports Export',
                htmlContent: htmlTable,
                attachment: [
                    {
                        content: base64Excel,
                        name: `Filtered_RM_Quality_Reports_${new Date().toISOString().split('T')[0]}.xlsx`
                    }
                ]
            });
            ...
            res.json({
                success: true,
                message: `Filtered RM Quality Reports mailed successfully to ${recipientEmail}`,
                data: {
            ...
            console.error('Error mailing filtered RM Quality Reports:', error);
            res.status(500).json({
                success: false,
                error: 'Failed to mail filtered RM Quality Reports',
            });
        }
    }
}
