...
exports.BatchMailFilteredController = void 0;
const prisma_1 = require("../../generated/prisma");
const exceljs_1 = __importDefault(require("exceljs"));
const brevomail_1 = require("../../service/brevomail");
const prisma = new prisma_1.PrismaClient();
class BatchMailFilteredController {
    // Mail filtered batches with Excel attachment
    static async mailFilteredBatches(req, res) {
        try {
            ...
            if (batchNumber)
                filtersApplied.push(`Batch: ${batchNumber}`);
            if (status)
                filtersApplied.push(`Status: ${status}`);
            if (productId)
                filtersApplied.push(`Product ID: ${productId}`);
            if (dateFrom)
                filtersApplied.push(`From: ${dateFrom}`);
            if (dateTo)
                filtersApplied.push(`To: ${dateTo}`);
...
exports.BatchMailFilteredController = BatchMailFilteredController;
