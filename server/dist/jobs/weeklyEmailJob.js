...
exports.runWeeklyEmailJob = void 0;
    ...
    try {
        ...
        for (const training of upcomingTrainings) {
            ...
            const participantEmails = training.participants
                .filter((p) => p.participant?.email)
                .map((p) => ({ email: p.participant.email, name: p.participant.name }));
...
const sendAuditReminders = async () => {
    let sent = 0;
    let errors = 0;
    try {
        const nextWeek = new Date();
        ...
        for (const audit of audits) {
            ...
            try {
                // Send to CLIENT_EMAIL for testing/notifications
                await (0, auditnotification_1.sendCustomNotification)({
...
const sendCorrectiveActionReminders = async () => {
    let sent = 0;
    let errors = 0;
    try {
        const nextWeek = new Date();
...
const sendBatchWeeklySummary = async () => {
    let sent = 0;
    let errors = 0;
    try {
        ...
        const statusCounts = batchStats.reduce((acc, stat) => {
...
const sendFindingReminders = async () => {
    let sent = 0;
    let errors = 0;
    try {
...
exports.runWeeklyEmailJob = runWeeklyEmailJob;
exports.default = exports.runWeeklyEmailJob;
